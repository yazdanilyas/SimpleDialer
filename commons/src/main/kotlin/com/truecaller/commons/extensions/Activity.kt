...
import com.truecaller.commons.models.*
import com.truecaller.commons.views.MyTextView
...
fun BaseSimpleActivity.isShowingSAFDialog(path: String): Boolean {
    return if (isPathOnSD(path) && !isSDCardSetAsDefaultStorage() && (baseConfig.treeUri.isEmpty() || !hasProperStoredTreeUri(
            false
        ))
    ) {
        runOnUiThread {
...
fun BaseSimpleActivity.getFileOutputStreamSync(
    ...
    return if (needsStupidWritePermissions(path)) {
        ...
        try {
            val newDocument =
                documentFile.createFile(mimeType, path.getFilenameFromPath()) ?: getDocumentFile(
                    path
                )
