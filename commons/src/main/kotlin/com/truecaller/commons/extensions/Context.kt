package com.truecaller.commons.extensions
...
import com.truecaller.commons.R
import com.truecaller.commons.helpers.*
import com.truecaller.commons.helpers.MyContentProvider.Companion.COL_APP_ICON_COLOR
import com.truecaller.commons.helpers.MyContentProvider.Companion.COL_BACKGROUND_COLOR
import com.truecaller.commons.helpers.MyContentProvider.Companion.COL_LAST_UPDATED_TS
import com.truecaller.commons.helpers.MyContentProvider.Companion.COL_NAVIGATION_BAR_COLOR
import com.truecaller.commons.helpers.MyContentProvider.Companion.COL_PRIMARY_COLOR
import com.truecaller.commons.helpers.MyContentProvider.Companion.COL_TEXT_COLOR
import com.truecaller.commons.models.AlarmSound
import com.truecaller.commons.models.BlockedNumber
import com.truecaller.commons.models.SharedTheme
import com.truecaller.commons.views.*
import java.io.File
    ...
    cursor?.use {
        if (cursor.moveToFirst()) {
            ...
            val navigationBarColor =
                cursor.getIntValueOrNull(COL_NAVIGATION_BAR_COLOR) ?: INVALID_NAVIGATION_BAR_COLOR
...
fun Context.getMyContactsContentProviderCursorLoader() =
...
//    val newAlarmSoundId = (yourAlarmSounds.max<T, R> { it.id }?.id ?: YOUR_ALARM_SOUNDS_MIN_ID) + 1
...
fun Context.getDuration(path: String): Int? {
    ...
    return try {
        ...
        retriever.setDataSource(path)
        Math.round(
            retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION).toInt() / 1000f
        )
    } catch (ignored: Exception) {
        null
    }
...
fun Context.isDefaultDialer(): Boolean {
    return if (!packageName.startsWith("truecaller.caller.callerid.name.phone.dialer.app") && !packageName.startsWith(
            "truecaller.caller.callerid.name.phone.dialer.app"
        )
    ) {
        true
    } else if ((packageName.startsWith("truecaller.caller.callerid.name.phone.dialer.app") || packageName.startsWith(
            "truecaller.caller.callerid.name.phone.dialer.app"
        )) && isQPlus()
    ) {
