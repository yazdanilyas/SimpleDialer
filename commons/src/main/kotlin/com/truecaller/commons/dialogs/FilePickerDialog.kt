...
import com.truecaller.commons.adapters.FilepickerItemsAdapter
import com.truecaller.commons.extensions.*
import com.truecaller.commons.helpers.SORT_BY_SIZE
import com.truecaller.commons.helpers.ensureBackgroundThread
import com.truecaller.commons.models.FileDirItem
import com.truecaller.commons.views.Breadcrumbs
...
class FilePickerDialog(
    ...
    private fun tryUpdateItems() {
        ensureBackgroundThread {
            getItems(
                currPath,
                activity.baseConfig.getFolderSorting(currPath) and SORT_BY_SIZE != 0
            ) {
        ...
        mDialogView.apply {
            ...
            filepicker_fastscroller.setViews(filepicker_list) {
                filepicker_fastscroller.updateBubbleText(
                    items.getOrNull(it)?.getBubbleText(context, mDateFormat, mTimeFormat) ?: ""
                )
            }
        ...
        for (file in files) {
            ...
            items.add(
                FileDirItem(
                    ...
                    curName,
                    file.isDirectory,
                    file.getDirectChildrenCount(showHidden),
                    size,
                    file.lastModified()
                )
            )
